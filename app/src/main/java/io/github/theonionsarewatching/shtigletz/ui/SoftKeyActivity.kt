...
open class SoftKeyActivity : ScaledActivity() {
    ...
    /**
     * Intercept BEFORE the focused view sees the event. Activity.onKeyDown
     * only fires when no child consumed the key — a focused, scrolling
     * WebView consumes keys, which made soft keys dead while reading a
     * message. dispatchKeyEvent runs first, so soft keys now work no matter
     * which view has focus. Only the two resolved codes are touched; every
     * other key passes straight through.
     */
    override fun dispatchKeyEvent(event: KeyEvent): Boolean {
        val keys = SoftKeys.resolve(this)
        if (keys != null) {
            val action: (() -> Unit)? = when (event.keyCode) {
                keys.first -> leftAction
                keys.second -> rightAction
                else -> null
            }
            if (action != null) {
                if (event.action == KeyEvent.ACTION_DOWN && event.repeatCount == 0) action()
                return true // consume DOWN and UP so nothing else reacts
            }
        }
