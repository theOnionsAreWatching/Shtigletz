...
class MessageListActivity : SoftKeyActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // Soft keys (when enabled): most-used actions for this screen.
        setSoftKeys(
            getString(R.string.list_compose), { compose() },
            getString(R.string.list_folders), { pickFolder() }
        )
    ...
    /** Visible whenever older mail may exist — including before the first
     *  successful refresh (cache-only view), so it shows right away. */
    private fun updateLoadOlderVisibility() {
        val ws = windowStart
        loadOlder.visibility =
            if (adapter.itemCount > 0 && (ws == null || ws > 1)) View.VISIBLE else View.GONE
    }
    ...
    private fun loadOlderPage() {
        val before = windowStart ?: run {
            // Cache-only view (e.g. fresh app start): sync first to learn the
            // server window, then this is re-invoked automatically.
